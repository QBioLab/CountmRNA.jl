...
data_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal_again/20201216";
ret_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal_again/20201216_result";
...
function playing(s)
	...
	for index in 1:length(longlived_labels)
        ...
        save("$ret_dir/d16$(s)-$index.jld", "threshold", nucleus_th, "normal", nor_para,
             "livingtime", livingtime[index], "track", tracks[:,:,index], "size", nucleus_size)
        save2ims(reinterpret.(cell_nu_nor), "$ret_dir/d16s$(s)-$index.ims", compression=2)
	end
	GC.gc()
end

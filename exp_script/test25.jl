...
data_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal/20200225-11-2_split";
ret_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal/20200225_result";

function playing(s)
    println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    println("$(Dates.now()) Loading $s")
	#@time img = load(File(format"TIFF","$data_dir/HE7-11-1-80uw-const_3_s$s.ome.tiff"));
    @time img = loadims("$data_dir/HE7-1NLS -11uW-cons_s$(s)_.ims");
	GC.gc()
	println("Load done")
	...
	for index in 1:length(longlived_labels)
        ...
        save("$ret_dir/d25$(s)-$index.jld", "threshold", nucleus_th, "normal", nor_para,

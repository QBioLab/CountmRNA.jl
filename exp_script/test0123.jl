...
data_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal_III/20210123";
ret_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal_III/2021023_result";
...
function playing(s)
	...
	for index in 1:length(longlived_labels)
        ...
        save("$ret_dir/d23s$(s)-$index.jld", "threshold", nucleus_th, "normal", nor_para,
             "livingtime", livingtime[index], "track", tracks[:,:,index], "size", nucleus_size)
        save2ims(reinterpret.(cell_nu_nor), "$ret_dir/d23s$(s)-$index.ims", compression=2)
	end
	GC.gc()
end

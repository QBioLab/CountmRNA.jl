...
data_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal/20200307-11-2-piezo";
ret_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal/20200307_result";

function playing(s)
    println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    println("$(Dates.now()) Loading $s")
	#@time img = load(File(format"TIFF","$data_dir/HE7-11-1-80uw-const_3_s$s.ome.tiff"));
    img = loadims("$data_dir/HED-1NLS-11-2-11uW-const-200ms_s$(s)_.ims")[12:500, 12:500, :, :];
	println("Load done")
	...
	for index in 1:length(longlived_labels)
        ...
        save("$ret_dir/d07$(s)-$index.jld", "threshold", nucleus_th, "normal", nor_para,
             "livingtime", livingtime[index], "track", tracks[:,:,index], "size", nucleus_size)
        save2ims(reinterpret.(cell_nu_nor), "$ret_dir/d07s$(s)-$index.ims", dx=0.16, compression=0)
	end
	GC.gc()
end

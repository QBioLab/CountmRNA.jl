...
data_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal_again/20201217";
ret_dir = "/datahub/rawdata/tandeng/mRNA_imaging/mRNA_confocal_again/20201217_result_new";
...
function playing(s)
	...
	@time time_line, longlived_labels, livingtime, time_line_whole = 
		find_time_line(markers, shortest=90);
	...
	for index in 1:length(longlived_labels)
        ...
        save("$ret_dir/d17$(s)-$index.jld", "threshold", nucleus_th, "normal", nor_para,
             "livingtime", livingtime[index], "track", tracks[:,:,index], "size", nucleus_size)
        save2ims(reinterpret.(cell_nu_nor), "$ret_dir/d17s$(s)-$index.ims", compression=2)
	end
	GC.gc()
end
